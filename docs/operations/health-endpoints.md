...
- **`/live`** — "¿el proceso está vivo?". Siempre devuelve `200` si el proceso puede responder HTTP, sin chequear dependencias externas. Lo usa el orquestador (Kubernetes/Docker) como _liveness probe_ para decidir si reiniciar el proceso. Reiniciar no arregla una DB caída, por eso `/live` no depende de eso.
- **`/ready`** — "¿puede recibir tráfico/trabajo?". Chequea las dependencias críticas (DB, Redis, o la conexión al servidor según el servicio) y devuelve `503` si alguna falla. Lo usa el orquestador como _readiness probe_, sin reiniciar el proceso — solo deja de enviarle tráfico hasta que vuelva a dar `200`.
